...
def perform_real_time_prediction():
    ...
    def generate_audio_data(emotion, duration, sample_rate):
        ...
        if emotion == 'happy':
            ...
            audio_data = librosa.core.tone(frequency, sr=sample_rate, duration=duration) + 0.3 * np.sin(2 * np.pi * 0.5 * t)
        else:
    ...
    def fetch_image_with_alpha(url):
        try:
            ...
            if response.status_code == 200:
                ...
                if image is not None and image.shape[2] == 3:  # If the image has 3 channels (no alpha), add an alpha channel
